...
class Duolingo:
    ...
    def __post_init__(self) -> None:
        """
        Post initialization, this class will try to log in into your Duolingo account with credentials your credentials
        to fetch your data before placing them in `user_data` and `daily_progress` for future processing.
        """
        # A "constructor" -- trying to log in the first time the class is initialized, to
        # abstract the whole 'login' process. We will try to check if the user has a JWT or
        # not, if not, then we will log them in the usual way.
        if not self.jwt:
            self.jwt = self.login()
    ...
    def get_streak_info(self) -> dict[str, str]:
        ...
        """
        return {
            "daily_goal": self.user_data["daily_goal"],
